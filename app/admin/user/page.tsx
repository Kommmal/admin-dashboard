      ...
      <div className="overflow-x-hidden px-3 py-6 bg-gray-100 min-h-screen">
              ...
              <tbody>
                {filteredUsers.map((user) => (
                  <tr key={user.id} className="border-b text-center">
                    ...
                    <td className={`md:py-4 md:px-2 p-1 ${user.status === "Active" ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}>{user.status}</td>
        ...
        <div className="flex justify-between items-center gap-2 mt-4">
                <button
                  ...
                  className={`md:px-3 md:text-sm text-xs px-2 py-2 rounded-md md:block hidden ${currentPage === 1 ? "bg-gray-300 cursor-not-allowed" : "bg-black text-white"}`}
                >
                  Previous
                </button>
                <button
                  onClick={() => paginate(currentPage - 1)}
                  disabled={currentPage === 1}
                  className={`md:px-3 md:text-sm text-xs px-2 py-2 rounded-md md:hidden block ${currentPage === 1 ? "bg-gray-300 cursor-not-allowed" : "bg-black text-white"}`}
                >
                  <ChevronDownIcon size={13} className="rotate-[90deg] md:hidden" />
                </button>
                <div className="flex gap-4">
                  ...
                  {[...Array(totalPages)].map((_, index) => (
                    <button
                      ...
                      className={`md:px-3 md:text-sm text-xs px-2 py-1 md:py-2 rounded-md lg:block ${currentPage === index + 1 ? "bg-black text-white" : "bg-gray-200"}`}
                    >
                      {index + 1}
                    </button>
                  ))}
                ...
                <button
                  ...
                  className={`md:px-3 md:text-sm text-xs px-2 py-2 rounded-md md:block hidden ${currentPage === totalPages ? "bg-gray-300 cursor-not-allowed" : "bg-black text-white"}`}
                >
                  Next
                </button>
                <button
